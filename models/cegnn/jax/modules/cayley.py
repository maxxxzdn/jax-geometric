...
class WeightedCayleyElementWise(nn.Module):
    """
    Parameterized Clifford multiplication table (element-wise).

    Attributes:
        algebra (object): An instance of CliffordAlgebra defining the algebraic structure.
        features (int): The number of input channels.
        product_paths_sum (int): The number of non-zero elements in the Cayley table.
            - given by algebra.geometric_product_paths.sum().item()
    """
    ...
    def __call__(self):
        ...
        # Initializing the weights
        stddev = 1 / jnp.sqrt(self.algebra.dim + 1)
        weight_init = self.param(
            "weight",
            normal(stddev),
            (self.features, self.product_paths_sum),
        )
